...
public class ManageCandidate extends javax.swing.JFrame {
    ...
    private void BrowsePhotoActionPerformed(java.awt.event.ActionEvent evt) {
        FileDialog fd = new FileDialog(new Frame(), "Select Photo", FileDialog.LOAD);
   ...
   private ImageIcon ResizePhoto(String ImagePath, byte[] pic) {
    ...
    // Determine the scale.
    double scaleFactor = Math.min(1.0 * CPhotoLabel.getWidth() / img.getWidth(null), 1.0 * CPhotoLabel.getHeight() / img.getHeight(null));
    ...
    Image newimg = img.getScaledInstance(scaleWidth, scaleHeight, Image.SCALE_SMOOTH);
